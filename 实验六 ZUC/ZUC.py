...
def rotl_uint31(a, shift):#a为31比特时循环左移shift位
    return ((a << shift) | (a >> (31 - shift))) & 0x7FFFFFFF#确保数据为31位时，注意使用&0x7FFFFFFF，
 
 
def rotl_uint32(a, shift):#a为32比特时循环左移shift位
    return ((a << shift) | (a >> (32 - shift))) & 0xFFFFFFFF#>>a 右移a位，丢掉移走的数据，也就是与原数据相比少了最右边a个数据
...
class ZUC(object):
    ...
    def bit_reorganization(self):#比特重组
    ...
    def zuc_init(self, key, iv):#初始化阶段
        # Expand key.
...
if '__main__' == __name__:
    key = [0x00]*16#128比特初始密钥
    iv  = [0x00]*16 #128比特初始向量
    zuc = ZUC(key, iv)
    # 加密过程
    out = zuc.zuc_encrypt(b"1")
